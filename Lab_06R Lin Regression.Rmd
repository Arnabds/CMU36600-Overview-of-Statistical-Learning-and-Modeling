...
```
FILL ME IN
The residual follows more or less standard normal(t) distribution. Then we can pull the information that we can reject the null hypothesis for Intercept, interventions, ERVisit, complication, comorbidities and duration which are informative.
```
...
plot(df.test$Cost, pred.lm, xlim=c(0,14), ylim=c(0,14))
abline(a=0,b=1)
```
```
FILL ME IN
I feel that it deviates when $x<4$ and $x>8$
```
...
```
FILL ME IN
All the values are less that 5 or 6. Hence there is no colinearity.
```
