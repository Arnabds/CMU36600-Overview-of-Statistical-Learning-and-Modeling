...
fit.fun <- function(par,data){
  model <- (par[1]*(data$x)^3+(par[2]*(data$x)^2)+(par[3]*data$x)^2)
...
ggplot(data=df,mapping=aes(x=x,y=y)) +
  geom_errorbar(aes(ymin=y-e, ymax=y+e), width=.1,color="blue") +
  geom_point(color="firebrick") + geom_line(mapping=aes(x=mod.x,y=mod.y),color="olivedrab")
```
...
ggplot(data=df,mapping=aes(x=x,y=y)) +
  geom_errorbar(aes(ymin=y-e, ymax=y+e), width=.1,color="blue") +
  geom_point(color="firebrick") +
  geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted),color="olivedrab")+ geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted+p$se),color="purple",linetype="dashed")+ geom_line(mapping=aes(x=loess.out$x,y=loess.out$fitted-p$se),color="purple",linetype="dashed")
```
