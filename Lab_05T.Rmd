...
```
FILL ME IN
distance is $d(R_1,R_2)=\sqrt{3}$, $d(R_1,R_3)=\sqrt{12}=2\sqrt{3}, d(R_3,R_2)=\sqrt{3}$ in both the cases.
```
...
# FILL ME IN
library(dplyr)
df.new=filter(df, dec > 56, ra < 264 , -350 < velocity.los & velocity.los< -250) %>%
  mutate(, gr=mag.g-mag.r, ri=mag.r-mag.i)
df3=df.new %>%
  select(-mag.i,-mag.g,-mag.r,-velocity.los)
...
km.out <- kmeans(scale(df3),2,nstart=20) # run clustering in the standardized space...
...
km.out <- kmeans(scale(df3),2,nstart=1)
...
km.out <- kmeans(scale(df3),3,nstart=100)
print(km.out)# run clustering in the standardized space...
#ggpairs(data=df3,mapping=aes(color=factor(km.out$cluster)))
# color <- km.out$cluster
# ggplot(data=df,mapping=aes(x=x,y=y)) + # ...but visualize in the native space
...
```
FILL ME IN
We have 3 groups with 248, 10 and 860 data in each cluster. The ratio is 33.9%.
```
...
wss <- rep(NA,10)
for ( ii in 1:10 ) { 
  km.out <- kmeans(scale(df3),ii,nstart=20); wss[ii] <- km.out$tot.withinss;
}
plot(1:10,wss,xlab="k",ylab="Within-Cluster Sum-of-Squares",pch=19,col="blue",typ="b")
```
I am confuse between k=2 and k=4.
...
ss <- rep(NA,10)
for ( ii in 2:10 ) { 
  km.out <- kmeans(scale(df3),ii,nstart=20); ss[ii] <- mean(silhouette(km.out$cluster,dist(scale(df3)))[,3]);
}
...
set.seed(303)
s <- sort(sample(nrow(df.new),1000))
# s you can run this code to know the difference between s and slice(df.new,s)
# FILL ME IN
slice(df.new,s)
```
...
```{r}
set.seed(202)
# FILL ME IN; try install.packages("factoextra", repos = "https://cran.rstudio.com/")
library(factoextra)
library(cluster)
...
set.seed(202)
# FILL ME IN
plot(df.new$price , ylab=df.new$carat )
```
It seems k=2 here.