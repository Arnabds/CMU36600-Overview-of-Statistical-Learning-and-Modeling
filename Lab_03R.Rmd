...
length() returns the no of columns of the data frame.
```
...
```
FILL ME IN WITH WORDS
Yes, u,g,z and i have maximum 99 which is probably the missing NA value and it is probably messing up the mean as well.
```
...
```{r}
# FILL ME IN
filter(df,u==99 | g==99 | z==99| y==99) %>%
nrow(.)
#which(df,u==99 | g==99 | z==99| y==99)
```
Even without nrow() we get the data where no of roows is written in a faded subtitle in the table.
## Question 7
...
```{r}
# FILL ME IN
pred.new=filter(df,u!=99 & g!=99 & z!=99& y!=99)
pred.new
```
...
```{r}
# FILL ME IN
w=which(df$u==99| df$g==99 | df$z==99 | df$z==99)
resp.new=response[-w]
resp.new
```
...
```{r}
# FILL ME IN
df<- pred.new %>% group_by(.,type) %>% summarize(., Number=n())
df
df=pred.new %>%
group_by(,type) %>%
summarize()
...
```{r}
# FILL ME IN
df<- pred.new %>% group_by(.,type) %>% summarize(., Median=median(u))
df
```
...
mean(df1$gr)
mean(df1$ri)
df1 %>% group_by(., type) %>% select(., type, gr, ri) %>% summarize(., Mean_gr=mean(gr), Mean_ri=mean(ri))
```

## Question 13

Let's go back to hypothesis testing for a moment. Let's extract two vectors of data from `df1`:
```{r}
gr.faint = df1$gr[df1$type=="FAINT"]
gr.bright = df1$gr[df1$type=="BRIGHT"]
gr.faint
gr.bright
```
...
hist(gr.faint)
hist(gr.bright)
t.test(gr.bright,gr.faint)
```
```
FILL ME IN WITH WORDS
From the two sample t test we get that the population means are not same with 0.05 level of significance as the p-value$=2.2*10^{-16}<<0.025$.
Nevertheless, we shouldn't use t test here as the data of gr.bright and gr.faint are not normally distributed. 
```
