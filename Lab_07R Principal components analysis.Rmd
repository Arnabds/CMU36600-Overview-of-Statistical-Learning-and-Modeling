...
```{r}
df.original <- read.csv("http://www.stat.cmu.edu/~pfreeman/breastcancer.csv",stringsAsFactors=TRUE)
response <- df.original[,1]  # B for benign, M for malignant
df <- df.original[,-1]
```
...
v <- pca.out$sdev^2
r<-round(cumsum(v/sum(v)),3)
#Based on this, I would retain 6 PCs

plot(r, xlab=" Principal Component ", ylab =" 
...
# FILL ME IN
library(ggfortify)
autoplot(pca.out, data=df.original, colour='Diagnosis')
```
...
```{r}
# FILL ME IN
autoplot(pca.out, x=2, y=3, data=df.original, colour='Diagnosis')
autoplot(pca.out, x=3, y=4, data=df.original, colour='Diagnosis')
```
...
df7<- ggplot(data=pca.out, aes(x=factor(df.original$Diagnosis), y=pca.out$x[,1])) + 
geom_boxplot(aes(fill=factor(df.original$Diagnosis)), outlier.shape=NA, position = dodge) + scale_y_continuous(lim=c(0, NA)) +
geom_jitter(position=position_jitterdodge(dodge.width=0.9), aes(color = factor(df.original$Diagnosis)), size=1.2, show.legend=FALSE) 
df7
```
...
```{r}
n <- 4                                             
df <- data.frame(pca.out$x[,1:6])
glm.out <- suppressWarnings(glm(response~.,data=df,family=binomial)) 
glm.prob <- predict(glm.out,type="response")
glm.pred <- ifelse(glm.prob>0.5,"M","B")
(t <- table(glm.pred,response))
cat("The misclassification rate for 6 Pcs is ",round((t[1,2]+t[2,1])/sum(t),3),"\n")
```
Extra PCs doesn't affect general linear model rate or read.