...
```{r}
# FILL ME IN
z=pnorm(5,mean=5,sd=2)
z
```
Yes, the result supports the claim that normal distru=bution is symmetric w.r.t the mean.
## Question 3
...
```{r}
# FILL ME IN
pnorm(7,mean=4,sd=2) - pnorm(3,mean=4,sd=2)
pnorm(9,mean=4,sd=2) - pnorm(1,mean=4,sd=2)
pnorm(11,mean=4,sd=2) - pnorm(-1,mean=4,sd=2)
```
...
plot(x,y, type = "p",col="purple")
plot(x,y, type = "o",col="green")
plot(x,y, type = "b",col="yellow")
plot(x,y, type = "h",col="brown")
z=dt(x, df=3)
plot(x,z)
lines(x,z,col="red")
```
...
x= 0:20
plot(x,dbinom(x, size=20,prob = 0.1))
x= 0:20
plot(x,dbinom(x, size=20,prob = 0.9))
# Not all of them. p=0.5, is similar to normal distribution. But, p=0.1(0.9) is
#left(right) skewed.
```
...
Yes, as the value of $\lambda$ grows bigger the distribution is getting closer to normal distribution.
...
#Another represtation
ppois(6, lambda=4.5, lower=FALSE)
```
...
integrate(integrand,0,1,a=0,b=1)  
...
*One footnote for me in future. If I want to integrate constant then I have to use $x^0$*

```{r}
integrand = function(x) {
  return(x^0)  # for the mean, we integrate x*f(x)
}
integrate(integrand,0,1)  # the value should be 1/beta ... which is what is output
```