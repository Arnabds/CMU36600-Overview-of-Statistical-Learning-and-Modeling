...
# FILL ME IN
ggplot(data=df,mapping=aes(x=redshift)) +
  geom_histogram(bins=60, color="papayawhip", fill="hotpink")
```
...
```{r}
# FILL ME IN
GalaxyPlot=ggplot(data=df,mapping=aes(redshift)) +
  geom_histogram(bins=60, color="papayawhip", fill="hotpink")
#Number of Galaxies at Redshift z
print(GalaxyPlot+ggtitle("Redshift Distribution for Buzzard DC1 Data")+labs(y= "Number of Galaxies at Redshift z", x = "Redshift z"))
```
...
ggplot(data=df,mapping=aes(x=u)) + 
  ...
  facet_wrap(~type,scales='free_y',s)+coord_cartesian(xlim = c(20, 32))
```
...
dfnew=df %>%
  ...
  gather()
#dfnew (I omiting to have thousands lines printed in html, but it is good to run this code to check whether my code is correct or not!)

df7=dfnew %>%
  ggplot( mapping=aes(x=value))+
  geom_histogram(color="blue",fill="springgreen",bins=25) +
  facet_wrap(~key,scales='free_y',s)+coord_cartesian(xlim = c(20, 32))
print(df7+ggtitle("New Faceted Histogram")+labs(x = "Magnitude"))
...
#Another code that is merely a change of my first code
#dfnew=df %>%
#  dplyr:: select(u,g,r,i,z,y) %>%
#  gather()
#dfnew (I omiting to have thousands lines printed in html, but it is good to run this code to check whether my code is correct or not!)
...
#  ggplot( mapping=aes(x=value))+
#  geom_histogram(color="blue",fill="springgreen",bins=25) +
#  facet_wrap(~key)+scale_x_continuous(lim = c(20, 32))
#print(df7+ggtitle("New Faceted Histogram")+labs(x = "Magnitude"))
#ggplot(data=df,mapping=aes(x=u)) + 
  #geom_histogram(color="blue",fill="yellow",bins=25) +
  #facet_wrap(~type,scales='free_y',s)+coord_cartesian(xlim = c(20, 32))
...
```{r}
# FILL ME IN
d8=ggplot(data=df,mapping=aes(x=r,y=redshift)) + 
  geom_point(color="red2",size=0.5)
d8+labs(title="Redshift vs 'r'-band magnitude",x="r-band magnitude", y="Redshift")
```
