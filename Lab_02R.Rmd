...
plot(x,fx)
z=(5-6)/(4/10)
plot(x,fx, type="l", col="blue", lwd=2,xlab="x", ylab="f(x)")
...
We fail to reject null hypothesis here. 
## Question 4
...
We reject the null hypothesis but by a small margin.
## Question 5
...
I observed that the p value are distributed uniformly. Here in a standard normal distribution, the mean is actually 0. Hence the null hypothesis is true and we see that p value are distributed uniformly. So it is plausible to conclude that when the null hypothesis is true, the $p$-values are distributed uniformly.
```
...
The p values are not uniformly distributed. The p-value is very small so we can reject null hypothesis( here I am considering 5% significance two sided).
...
We will reject the null hypothesis, which is mean $\mu_0=10.5$, with 0.05 level of significance.
...
t.test(x,y, var.eq =F, paired=F)
```

We will reject the null hypothesis that means there is evidence that the means are different.
## Question 11
...
So we fail to reject the null hypothesis as p-value$=0.4335 >> 0.05$ with $0.05$ level of significance.
